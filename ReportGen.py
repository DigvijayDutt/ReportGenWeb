...
class MultiDocApp:
    def __init__(self):
        ...
        ui.label('📑 Multi Document Generator').classes('text-2xl font-bold text-blue-700 mb-4')

        with ui.column().classes('gap-4 w-1/2 mx-auto'):
            self.render_widgets()
    ...
    def render_widgets(self):  
        with ui.row().classes('gap-6 w-full justify-between'):
            ...
            with ui.column().classes('items-start'):
                ...
                ui.upload(on_upload=self.handle_folder_zip_upload, auto_upload=True).props('accept=.zip')
...
# Run app
MultiDocApp()
ui.run(title='Multi Document Generator', port=8080)